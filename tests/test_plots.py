...
def test_bruker1d_multiple():
    ...
    fig, ax = bruker1d([DATA_DIR_1D_1, DATA_DIR_1D_2], labels=["s1", "s2"], return_fig=True)
    assert fig is not None
    assert ax is not None
...
def test_bruker1d_save(tmp_path):
    ...
    bruker1d(DATA_DIR_1D_1, save=True, filename=str(output_file.with_suffix('')), format='png')
    assert output_file.is_file()
    assert output_file.stat().st_size > 0 # Check file is not empty

def test_bruker2d_single():
    """Test basic 2D plot."""
    ax_dict = bruker2d(DATA_DIR_2D, contour_start=1e5, contour_num=10, contour_factor=1.5, return_fig=True)
...
def test_bruker2d_save(tmp_path):
    ...
    bruker2d(DATA_DIR_2D, contour_start=1e5, contour_num=10, contour_factor=1.5, save=True, filename=str(output_file.with_suffix('')), format='png')
    assert output_file.is_file()
...
def test_bruker1d_background():
    ...
    fig, ax = bruker1d_background(DATA_DIR_1D_1, DATA_DIR_1D_2, background_factor=0.5, return_fig=True)
    assert fig is not None
    assert ax is not None
...
def test_df2d(tmp_path):
    ...
    ax_dict_csv = df2d(str(csv_path), contour_start=1e5, contour_num=10, contour_factor=1.5, return_fig=True)
    assert isinstance(ax_dict_csv, dict)
    assert 'A' in ax_dict_csv

    ax_dict_df = df2d(df_2d_data, contour_start=1e5, contour_num=10, contour_factor=1.5, return_fig=True)
